...
import { renderMenuItems } from "./modules/renderMenuItems.js";
import {
  ACTION_CONFIG,
  buildEllipsisPopup,
  hideScreenOverlay,
...
function unpairRemoteClient() {
  ...
  uiState.section = "overview";
  document
    .querySelectorAll(".menu-item")
    .forEach((i) => i.classList.remove("active"));
  document
    .querySelector('.menu-item[data-section="overview"]')
...
socket.on("display_state", ({ state }) => {
  ...
  // Theme1 Start
  uiState.data.filteredMenuItems = state?.filteredMenuItems || [];
  renderMenuItems();
  // Theme1 End
